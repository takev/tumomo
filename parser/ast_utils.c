...
size_t utf8_to_ucs4(const uint8_t *in, size_t in_size, uint32_t *out)
{
    off_t    i, j;
    uint8_t  c_in;
    uint32_t c_out = 0;
...
size_t string_escape(uint32_t *string, size_t string_size, int raw)
{
    uint32_t c;
    off_t    i, j;
    int      escape = 0;
    ...
    for (i = 0, j = 0; i < string_size; i++) {
        ...
        if (count) {
            ...
            if (c >= '0' && c <= '9') {
                ...
                ast_error("Could not decode hex escape character.");
                abort();
            }
